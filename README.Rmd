...
tsforecast is a package developed within ING and is not available on CRAN. 
...
Any required packages that are missing from your R library will be indicated during installation or loading of the package, please [install these missing packages](https://stat.ethz.ch/R-manual/R-devel/library/utils/html/install.packages.html) and retry installing/loading tsforecast.
...
You need to initialize the data to be used within the time series forecasting framework (using [`initialize_ts_forecast_data()`](#initialize_ts_forecast_data) from the tstools package), by specifying which columns correspond to which required fields for the forecasting:
```{r, eval = TRUE}
